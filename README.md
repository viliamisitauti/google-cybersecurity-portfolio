# Google Cybersecurity Portfolio

## About Me 
I am a High School student currently completing the Google Cybersecurity Professional Certificate on Coursera. This repository documents my hands-on cybersecurity learning, including security audits, labs, notes, and tools.

My goal is to build strong foundational cybersecurity skills and prepare for future certifications such as CompTIA Security+ while continuing to expand real-world technical experience.

## Repository Structure
- **audits/** – Internal security audits and risk assessments  
- **labs/** – Hands-on labs and practice exercises  
- **notes/** – Cybersecurity concepts and study notes  
- **tools/** – Scripts and security tools I practice with  
- **certificates/** – Completed certifications and achievements  

## Featured Project
### Internal Security Audit
- Conducted a checklist-based internal security audit of an organization  
- Reviewed security controls, policies, and risk areas  
- Documented findings and improvement recommendations  

## Skills Being Developed
- Risk assessment and security auditing  
- Documentation and reporting  
- Linux fundamentals  
- Networking basics  
- Security principles (CIA Triad, access control, incident response)  

## Goals
- Complete the Google Cybersecurity Professional Certificate  
- Build a strong cybersecurity portfolio  
- Pursue CompTIA Security+ certification  
- Gain hands-on experience in defensive security and SOC operations  

## Contact
GitHub Profile: https://github.com/YOUR_USERNAME
