# Google Cybersecurity Portfolio

## About Me
I am a student completing the Google Cybersecurity Professional Certificate on Coursera.  
This portfolio demonstrates my hands-on cybersecurity learning through audits, labs, notes, and tools.  
My goal is to build strong foundational cybersecurity skills and prepare for future certifications such as CompTIA Security+.

## Featured Project: Botium Toys Internal Security Audit 
- Conducted a checklist-based internal security audit of a fictional company, Botium Toys.  
- Evaluated security controls, policies, and compliance practices.  
- Documented findings, risks, and recommendations to improve security posture.  
- Files included:
  - `Botium_Toys_Internal_Audit_Checklist.pdf`  
  - `Botium_Toys_Audit_Summary.md`  

## Repository Structure
- **audits/** – Internal security audits and assessments  
- **labs/** – Hands-on labs and practice exercises  
- **notes/** – Cybersecurity concepts and study notes  
- **tools/** – Scripts and security tools I practice with  
- **certificates/** – Completed certifications and achievements  

## Skills Demonstrated
- Security auditing and risk assessment  
- Security controls evaluation  
- Documentation and reporting  
- Cybersecurity fundamentals (CIA triad, access control, incident response)  

## Contact
GitHub Profile: https://github.com/YOUR_USERNAME
